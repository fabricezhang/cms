...
<head>
    ...
    <title>CMS Modify Article</title>
...
<body>
<div class="container">
    <h1>CMS Modify Article</h1>
    <hr/>
    <form:form action="/admin/articles/updateP" method="post" commandName="articleP" role="form">
        ...
        <div class="form-group">
            ...
            <select class="form-control" id="userByUserId.id" name="userByUserId.id">
                <c:forEach items="${userList}" var="user">
                    <c:if test="${user.id==article.userByUserId.id}">
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" id="content" name="content" rows="3"
                      placeholder="Please Input Content">${article.content}</textarea>
        </div>
        <div class="form-group">
            <label for="pubDate">Publish Date:</label>
            <input type="date" class="form-control" id="pubDate" name="pubDate"
                   value="<fmt:formatDate value="${article.pubDate }" pattern="yyyy-MM-dd"/>"/>
        </div>
        <!-- 把 id 一并写入 articleP 中 -->
